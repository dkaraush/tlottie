...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let shader = Path::new("shaders/triangle.wgsl");
    ...
    let coverage_shader = Path::new("shaders/coverage.wgsl");
    ...
    let coverage_info = naga::valid::Validator::new(
        naga::valid::ValidationFlags::all(),
        naga::valid::Capabilities::all(),
    )
    .validate(&coverage_module)?;
    for (entry, file) in [
        ("main", "coverage.comp.spv"),
        ("simple_main", "coverage-simple.comp.spv"),
    ] {
        let coverage_pipeline = naga::back::spv::PipelineOptions {
            shader_stage: naga::ShaderStage::Compute,
            entry_point: entry.to_string(),
        };
        let coverage_words = naga::back::spv::write_vec(
            &coverage_module,
            &coverage_info,
            &options,
            Some(&coverage_pipeline),
        )?;
        let coverage_bytes = coverage_words
            .iter()
            .flat_map(|word| word.to_le_bytes())
            .collect::<Vec<_>>();
    ...
    let bins_shader = Path::new("shaders/bins.wgsl");
    ...
    let bins_info = naga::valid::Validator::new(
        naga::valid::ValidationFlags::all(),
        naga::valid::Capabilities::all(),
    )
    .validate(&bins_module)?;
    let bins_pipeline = naga::back::spv::PipelineOptions {
        shader_stage: naga::ShaderStage::Compute,
        entry_point: "main".to_string(),
    };
    let bins_words =
        naga::back::spv::write_vec(&bins_module, &bins_info, &options, Some(&bins_pipeline))?;
    let bins_bytes = bins_words
        .iter()
        .flat_map(|word| word.to_le_bytes())
        .collect::<Vec<_>>();
