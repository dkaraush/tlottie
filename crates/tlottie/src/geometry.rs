...
pub(crate) fn quad_contains_box(quad: &[Vec2; 4], x0: f32, y0: f32, x1: f32, y1: f32) -> bool {
    let area = (quad[1].x - quad[0].x) * (quad[2].y - quad[0].y)
...
pub(crate) fn extract_by_length(
    points: &[Vec2],
    anchors: &[bool],
    closed: bool,
    l0: f32,
    ...
    'outer: for round in 0..rounds {
        for i in 0..seg_count {
            ...
            out.push(p1);
            out_anchor
                .push(t1 >= 1.0 - 1e-6 && anchor_of(if i + 1 == points.len() { 0 } else { i + 1 }));
            emitted += hi - lo;
            if end <= occ_end {
                break 'outer;
            }
...
pub(crate) fn polystar_path(
    ...
    for i in 0..n {
        ...
        let (radius, roundness) = if outer_vertex {
            (outer_radius, out_round)
        } else {
            (inner_radius, in_round)
        };
        ...
        data.in_tangents
            .push(Vec2::new(-perp.x * tan_len, -perp.y * tan_len));
        data.out_tangents
            .push(Vec2::new(perp.x * tan_len, perp.y * tan_len));
        angle += angle_per;
    }
...
pub(crate) fn round_polyline_corners(c: &Contour, closed: bool, radius: f32) -> Contour {
    // (output keeps c.inv_lin — same flatten matrix)
    let pts = &c.points;
    let n = pts.len();
    if radius <= 0.0 || n < 3 {
        return c.clone();
    ...
    for i in range {
        ...
        if cos_turn > 0.985 {
            // Near-collinear: part of a flattened curve, keep as-is.
            out.push(here);
            continue;
        }
        let r = radius.min(l0 * 0.5).min(l1 * 0.5);
        let a = Vec2::new(here.x - d0.x / l0 * r, here.y - d0.y / l0 * r);
        let b = Vec2::new(here.x + d1.x / l1 * r, here.y + d1.y / l1 * r);
        // Approximate the arc with a few points along the quadratic bezier
        // a -> here -> b.
        out.push(a);
        for step in 1..4 {
    ...
    // Same flatten matrix as the source: keep its local measure.
    Contour {
        points: out,
        anchors: Vec::new(),
        inv_lin: c.inv_lin,
...
impl<'a> VDasher<'a> {
    ...
    fn move_to(&mut self) {
        ...
        if self.dash_offset.abs() > VDASH_EPS && !self.pairs.is_empty() {
            ...
            let mut n = if total_length > 0.0 {
                self.dash_offset % total_length
            } else {
                0.0
            };
            if n < 0.0 {
    ...
    fn process_element(&mut self, verts: &[(Vec2, bool)]) {
        ...
        let first = verts
            .first()
            .copied()
            .unwrap_or((Vec2::new(0.0, 0.0), true));
    ...
    fn run(&mut self) {
        ...
        for w in bounds.windows(2) {
            ...
            let verts: Vec<(Vec2, bool)> = (a..=b)
                .filter_map(|i| self.points.get(i).map(|p| (*p, self.anchor_of(i))))
                .collect();
