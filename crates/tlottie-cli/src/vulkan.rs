...
mod imp {
    ...
    impl VkCtx {
        fn new() -> Result<VkCtx, String> {
            ...
            let app_name = CStr::from_bytes_with_nul(b"tlottie-cli\0")
                .map_err(|e| format!("app name: {e}"))?;
            let engine_name =
                CStr::from_bytes_with_nul(b"tlottie\0").map_err(|e| format!("engine name: {e}"))?;
    ...
    impl GpuBuffer {
        fn new(
            ...
            let buffer = unsafe { ctx.device.create_buffer(&info, None) }
                .map_err(|e| format!("vkCreateBuffer({name}): {e:?}"))?;
    ...
    fn memory_type(
        ...
        Err(format!(
            "no memory type with flags 0x{:x} for {name}",
            properties.as_raw()
        ))
    }
    ...
    pub(crate) fn render_with_options(
        ...
        let Some(bytes) = (width as vk::DeviceSize)
    ...
    fn record_submit_read(
        ...
        let scratch_target = tlottie_vulkan::BufferTarget {
            buffer: scratch.buffer,
            width,
            height,
            bytes: scratch.bytes,
        };
        let image_target = tlottie_vulkan::ImageTarget {
            image: image.image,
            format: vk::Format::B8G8R8A8_UNORM,
            width: image.width,
            height: image.height,
        ...
        if let Err(e) =
            unsafe { renderer.record(cmd, scratch_target, image_target, comp, frame, options) }
        {
    ...
    fn create_timestamp_pool(ctx: &VkCtx) -> Option<vk::QueryPool> {
        ...
        let supports_timestamps = props
            .get(ctx.queue_family_index as usize)
...
#[cfg(feature = "vulkan")]
pub(crate) use imp::render_with_options;

#[cfg(not(feature = "vulkan"))]
pub(crate) fn render_with_options(
    _comp: &tlottie::Composition,
    _frame: f32,
    _pixels: &mut [u32],
    _width: u32,
    _height: u32,
    _options: tlottie::RenderOptions,
