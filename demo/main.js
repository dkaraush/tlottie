...
// behind the same tiny C ABI (crates/tlottie-wasm/src/lib.rs,
...
const engines = [
  {
    ...engineDom('tl'),
    alloc: (n) => tl.tl_alloc(n),
    free: (p, n) => tl.tl_free(p, n),
    heap: () => new Uint8Array(tl.memory.buffer),
    create: (p, n) => tl.tl_new(p, n),
    drop: (i) => tl.tl_drop(i),
    width: (i) => tl.tl_width(i),
    height: (i) => tl.tl_height(i),
    frameRate: (i) => tl.tl_frame_rate(i),
    frameCount: (i) => tl.tl_frame_count(i),
    render: (i, f, w, h, aa) => tl.tl_render(i, f, w, h, aa ? 1 : 0),
  },
  {
    ...engineDom('rl'),
...
const MAX_DIM = 1024;
const WINDOW = 120;  // rolling-average window, in draws
